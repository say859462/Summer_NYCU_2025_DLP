...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", type=int, default=168, help="Random seed for evaluation"
    )
    parser.add_argument(
    ...
    # Iterate seed to seed + 19
    all_rw = 0
    for i in range(20):
        reward = evaluate(args)
        print(
            "Iteration {} with seed {} ,Eval Reward: {}".format(
                i + 1, args.seed, reward
            )
        )
        all_rw += reward
        args.seed += 1

    print("Average reward: {:.4f}".format(all_rw / 20.0))
